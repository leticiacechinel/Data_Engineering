...
public class CidadeEstadoSegmentoTest {
	...
	public void testaMetricasCidadeEstadoSegmento () throws ParseException {
		...
		//Blumenau
		Metrica[] mCidade = servico.getMetricasCidade("Blumenau", 201904);
		Assert.assertEquals(6,mCidade.length);
		Metrica mc = getMetrica(mCidade, Tipo.MRR);
		Assert.assertEquals(200.0, mc.getValor(),0.001);
		mc = getMetrica(mCidade, Tipo.New_MRR);
		...
		//Métricas Estado
		Metrica[] mEstado = servico.getMetricasEstado("SC", 201904);
		Assert.assertEquals(6,mCidade.length);

		Metrica me = getMetrica(mEstado, Tipo.MRR);
		Assert.assertEquals(385.0, me.getValor(),0.001);

		me = getMetrica(mEstado, Tipo.New_MRR);
		Assert.assertEquals(100.0, me.getValor(),0.001);
		
		me = getMetrica(mEstado, Tipo.Contraction_MRR);
		Assert.assertEquals(65.0, me.getValor(),0.001);
		
		
        //Métrica Segmento
		Metrica[] mSegmento = servico.getMetricasSegmento("Software", 201904);
		Assert.assertEquals(6,mSegmento.length);
		
		Metrica ms = getMetrica(mSegmento, Tipo.MRR);
		Assert.assertEquals(499.0, ms.getValor(),0.001);

		 ms = getMetrica(mSegmento, Tipo.New_MRR);
		Assert.assertEquals(100.0, ms.getValor(),0.001);

		 ms = getMetrica(mSegmento, Tipo.Expansion_MRR);
		...
		Assert.assertEquals(6,mCidade.length);
	    ms = getMetrica(mSegmento, Tipo.MRR);
		Assert.assertEquals(100.0, mc.getValor(),0.001);
		...
		Assert.assertEquals(6,mCidade.length);
	    mc = getMetrica(mCidade, Tipo.Cancelled_MRR);
