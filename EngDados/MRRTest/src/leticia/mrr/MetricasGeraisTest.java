...
public class MetricasGeraisTest {
	...
	public void testaMetricas() throws ParseException {
		...
		servico.pagamentoEfetuado(5, "10/04/2019", "Platina/1", 399);
 		...
 		MetricaEmpresa[] metricas = servico.getMetricasEmpresa(201904);
		Assert.assertEquals(6, metricas.length);
		
		
		MetricaEmpresa me = getMetricaEmpresa(metricas,Tipo.MRR);
		Assert.assertEquals(1183.0, me.getValor(),0.001);
		
	    me = getMetricaEmpresa(metricas,Tipo.New_MRR);
		Assert.assertEquals(100.0, me.getValor(),0.001);
		
	    me = getMetricaEmpresa(metricas,Tipo.Expansion_MRR);
		Assert.assertEquals(214.0, me.getValor(),0.001);
		
	    me = getMetricaEmpresa(metricas,Tipo.Contraction_MRR);
		Assert.assertEquals(65, me.getValor(),0.001);
		
		
	    me = getMetricaEmpresa(metricas,Tipo.Cancelled_MRR);
		Assert.assertEquals(185.0, me.getValor(),0.001);
		
	    me = getMetricaEmpresa(metricas,Tipo.Resurrected_MRR);
		...
		Assert.assertEquals(6, metricas.length);
		 ...
		 me = getMetricaEmpresa(metricas,Tipo.MRR);
	...
	private MetricaEmpresa getMetricaEmpresa(MetricaEmpresa[] metricas, Tipo tipo) {
		for (MetricaEmpresa metricaEmpresa : metricas) {
			if (metricaEmpresa.getTipo() == tipo){
				return metricaEmpresa;
			}
		}
		return null;
	}
